...
export default function MailCard({ cardInfo, isfavorite , mailClickHandler  , isAlreadyreaded , mailSelected   }) {
  ...
  return (
    <div className={"flex rounded-2xl h-32 gap-5 p-4 cursor-pointer  shadow-md " +   ( isAlreadyreaded ?  "bg-white " : " bg-gray-200 " ) + ( ( parseInt(mailSelected) == id ) ? ' border border-[#E54065] ' : "")  } 
    onClick = { (  ) => { mailClickHandler( id )} }>
