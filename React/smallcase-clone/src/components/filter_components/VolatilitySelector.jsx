...
import { volatalityOptionsMapping } from "../../data/mappers.js"
...
export default function VolatilitySelector({
  ...
  return (
    <div>
      <h3 className="font-semibold text-gray-900 my-2 ">{"Volatility"}</h3>
        ...
        {Object.keys(volatalityOptionsMapping).map((volatality) => (
          <div key={volatality} className="grow w-1">
            <input
              ...
              checked={desiredVolatilities.has(
                volatalityOptionsMapping[volatality]
              )}
            />
            <label
              htmlFor={volatality}
              className=" flex flex-col pt-2 pb-1 rounded-sm gap-2 border border-gray-200 peer-checked:border-2 peer-checked:border-indigo-500 hover:bg-gray-100  justify-center items-center"
            >
