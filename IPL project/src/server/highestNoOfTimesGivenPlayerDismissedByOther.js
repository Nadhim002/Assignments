...
function highestNoOfTimesGivenPlayerDismissedByOther(path,fileNameToSave,playerName) {
  ...
  let mostDismisalStats = deliveriesData.reduce(
    function (acc, delivery) {
      if (
        delivery["player_dismissed"] === playerName &&
  ...
  for (let bowler in mostDismisalStats) {
    if (mostDismisalStats[bowler] > mostDismisalCount) {
      ...
      mostDismisalStats[bowler] === mostDismisalCount &&
      bowler < mostDismissedBowler
    ) {
      mostDismissedBowler = bowler;
    }
  }
  ...
  const outputObject = {
    [playerName]: { Bowler: mostDismissedBowler, Count: mostDismisalCount },
  };

  fs.writeFileSync(fileNameToSave, JSON.stringify(outputObject, null, 2));
}

highestNoOfTimesGivenPlayerDismissedByOther(
  "../data/deliveriesJsonData.json",
  "../public/output/highestNoOfTimesGivenPlayerDismissedByOther.json",
  "DA Warner"
);
