...
function matchesPerYear(path , fileNameToSave ){
  ...
  let outputObject  = matchData.reduce( 
    function(acc, row ){
        ...
        acc[ row["winner"] ][ String( row["season"] ) ] =  ( acc[ row["winner"] ][ String( row["season"] ) ] ?? 0 ) + 1 
...
matchesPerYear("../data/matchesJsonData.json" , "../public/output/matchesWonPerTeamPerYear.json")
