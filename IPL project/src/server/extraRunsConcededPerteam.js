...
function extraRunsConcededPerteam( path , fileNameToSave ){
  ...
  let outputObject = deliveryData.reduce(function(acc,row){
    ...
    if ( matchIdsForGivenYear.includes( row["match_id"] ) ){
        acc[ row["bowling_team"] ] = ( acc[ row["bowling_team"] ] ?? 0 ) + parseInt( row["extra_runs"] )
    }
...
extraRunsConcededPerteam( ["../data/matchesJsonData.json" , "../data/deliveriesJsonData.json" ], "../public/output/extraRunsConcededPerteam.json")
