...
function solutionFunction(inventory , id = 33 ){
    ...
    if ( typeof id !== 'number'){
        console.log("Enter id in proper fomat")
        return
    }
    ...
    for (let obj of inventory ){
        if ( obj.id === id ){
            console.log(`Car ${id} is a ${obj.car_year || "Car year not Available"} ${obj.car_make || "Car make not Available"}  ${obj.car_model || "Car model not Available"} `)
            return
        }
    }
